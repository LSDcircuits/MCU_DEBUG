...
def run_gui():
    ...
    class ScrollFrame(ttk.Frame):
        """Vertically scrollable container - build screens inside ``.inner``.

        Every screen gets a scrollbar plus mouse-wheel support, so long
        question lists, feedback and procedure checklists are always
        reachable no matter the window size.
        """
        ...
        def on_mousewheel(self, event):
            # Windows/macOS deliver <MouseWheel> with .delta;
            # X11 delivers <Button-4>/<Button-5> with .num.
            if getattr(event, "num", None) == 4:
                self.canvas.yview_scroll(-1, "units")
            elif getattr(event, "num", None) == 5:
                self.canvas.yview_scroll(1, "units")
            elif getattr(event, "delta", 0):
                self.canvas.yview_scroll(-1 if event.delta > 0 else 1,
                                         "units")
    ...
    class TrainerApp:
        ...
        def __init__(self, root):
            ...
            style.configure("Small.TLabel", font=self.FONT_SMALL,
                            foreground="#555")
            style.configure("Good.TLabel", font=self.FONT_BOLD,
                            foreground="#1a7a1a")
            style.configure("Bad.TLabel", font=self.FONT_BOLD,
                            foreground="#b02020")

            # One scrollable container for every screen.
            self.scroller = ScrollFrame(root)
            self.scroller.pack(fill="both", expand=True)
            self.container = self.scroller.inner
            root.bind_all("<MouseWheel>", self.scroller.on_mousewheel)
            root.bind_all("<Button-4>", self.scroller.on_mousewheel)
            root.bind_all("<Button-5>", self.scroller.on_mousewheel)

            # quiz state
            self.session = None
            self.quiz_title = ""
            self.quiz_index = 0
            # quiz_answers: question index -> [correct(bool), needs_review(bool)]
            self.quiz_answers = {}
            self.quiz_flushed = False
        ...
        def _clear(self):
            for child in self.container.winfo_children():
                child.destroy()
            self.scroller.scroll_top()

        def _header(self, title, subtitle=None):
            ttk.Label(self.container, text=title,
        ...
        def show_dashboard(self):
            ...
            if not bank.questions:
                ttk.Label(
                    ...
                    style="Small.TLabel", wraplength=900, justify="left",
                ).pack(anchor="w", pady=(0, 12))
            ...
            for row, (label, command) in enumerate(entries):
                ttk.Button(buttons, text=label, command=command,
                           width=28).grid(row=row, column=0, sticky="w",
                                          pady=4)

            summary = progress.summary()
            stats = ("Lifetime: %d attempt(s), %d correct, %d in review queue."
                     % (summary["attempts"], summary["correct"],
        ...
        def start_quiz_setup(self, fixed_type):
            ...
            self._clear()
            title = ("Practice Exam" if fixed_type is None
                     else "%s Drill" % fixed_type.title())
            self._header(title)
            ...
            ttk.Label(self.container,
                      text="%d question(s) in the pool." % len(pool)
                      ).pack(anchor="w", pady=4)
            row = ttk.Frame(self.container)
            row.pack(anchor="w", pady=4)
            ttk.Label(row,
                      text="Number of questions (blank = all):").pack(
                side="left")
            ...
            ttk.Button(self.container, text="Start",
                       command=go).pack(anchor="w", pady=8)
            ttk.Button(self.container, text="Back",
        ...
        def start_category_drill(self):
            ...
            ttk.Combobox(form, textvariable=cat_var, values=cat_values,
                         state="readonly", width=34).grid(row=0, column=1,
                                                          sticky="w")

            ttk.Label(form, text="Exercise:").grid(row=1, column=0,
                                                   sticky="w", pady=3)
            ex_values = ["(all)"] + bank.exercises()
            ex_var = tk.StringVar(value="(all)")
            ttk.Combobox(form, textvariable=ex_var, values=ex_values,
                         state="readonly", width=34).grid(row=1, column=1,
                                                          sticky="w")

            ttk.Label(form, text="Type:").grid(row=2, column=0,
                                               sticky="w", pady=3)
            type_values = ["(all)"] + list(core.QUESTION_TYPES)
            type_var = tk.StringVar(value="(all)")
            ttk.Combobox(form, textvariable=type_var, values=type_values,
                         state="readonly", width=34).grid(row=2, column=1,
                                                          sticky="w")
        ...
        def show_question(self):
            ...
            # feedback (shown once this question has been answered)
            self.feedback_area = ttk.Frame(self.container)
            self.feedback_area.pack(anchor="w", fill="x", pady=6)
            if self.quiz_index in self.quiz_answers:
                self._render_feedback()

            self._build_quiz_nav()

        # answer builders; each calls _grade(correct) on submit --------------
            ...
            def submit():
                ...
                if pos < 0:
                    messagebox.showinfo("Answer", "Pick a choice first.",
                                        parent=self.root)
                    return
                self._grade(core.grade_mcq(q, self.mcq_displayed[pos][0]))
        ...
        def _build_numeric(self, q):
            ...
            def submit():
                self._grade(core.grade_numeric(
            ...
            for part in q.get("parts") or []:
                ...
                unit = " (%s)" % part["unit"] if part.get("unit") else ""
                ttk.Label(row, text="%s%s:" % (part.get("label", "Value"),
                                               unit),
            ...
            if self.numeric_entries:
                first_row = self.answer_area.winfo_children()[0]
                first_row.winfo_children()[1].focus_set()
        ...
        def _build_sequence(self, q):
            ...
            def submit():
                ...
                if order is None:
                    ...
                    self._grade(core.grade_sequence(q, order))
        ...
        def _build_self_check(self, q):
            ...
            reveal_btn = ttk.Button(self.answer_area,
            ...
            yes_btn = ttk.Button(marks, text="I recalled it",
                                 command=lambda: self._grade(
                                     core.grade_self_check(q, True)))
            no_btn = ttk.Button(marks, text="Needs work",
                                command=lambda: self._grade(
                                    core.grade_self_check(q, False)))
            yes_btn.pack(side="left", padx=(0, 6))
            no_btn.pack(side="left")
            if self.quiz_index not in self.quiz_answers:
                yes_btn.state(["disabled"])
                no_btn.state(["disabled"])
            else:
                # already answered: show the model answer straight away
                reveal()
            self.submit_frame = None

        def _build_submit(self, submit):
            self.submit_frame = ttk.Frame(self.answer_area)
        ...
        def _grade(self, correct):
            """Store (or overwrite) the grade for the current question."""
            entry = self.quiz_answers.setdefault(self.quiz_index,
                                                 [bool(correct), False])
            entry[0] = bool(correct)
            self._render_feedback()

        def _render_feedback(self):
            for child in self.feedback_area.winfo_children():
                child.destroy()
            correct, needs_review = self.quiz_answers[self.quiz_index]
            q = self.session.questions[self.quiz_index]

            fb = self.feedback_area
            ...
            if q.get("source"):
                ttk.Label(fb, text="Source: %s" % q["source"],
                          style="Small.TLabel", wraplength=900,
                          justify="left").pack(anchor="w")

            flag_var = tk.BooleanVar(value=needs_review)

            def on_toggle(*_args, idx=self.quiz_index, var=flag_var):
                if idx in self.quiz_answers:
                    self.quiz_answers[idx][1] = bool(var.get())

            flag_var.trace_add("write", on_toggle)
            ttk.Checkbutton(
                fb, text="Needs review (queue this question even if correct)",
                variable=flag_var).pack(anchor="w", pady=4)

        # forward / backward navigation ----------------------------------------

        def _build_quiz_nav(self):
            nav = ttk.Frame(self.container)
            nav.pack(anchor="w", pady=10)
            last = len(self.session.questions) - 1
            ...
            next_label = "Finish >" if self.quiz_index >= last else "Next >"
            ttk.Button(nav, text=next_label,
                       command=self._next_question).pack(side="left", padx=8)
            ttk.Button(nav, text="End quiz",
                       command=self.show_end_screen).pack(side="left")
        ...
        def _next_question(self):
            if self.quiz_index < len(self.session.questions) - 1:
                self.quiz_index += 1
                self.show_question()
            else:
                self.show_end_screen()
        ...
        def _flush_results(self):
            """Record every answered question once into session + progress."""
            if self.quiz_flushed:
                return
            self.quiz_flushed = True
            for idx in sorted(self.quiz_answers):
                q = self.session.questions[idx]
                correct, needs_review = self.quiz_answers[idx]
                self.session.record(q, correct, needs_review)
                if q.get("id"):
                    progress.record_question(q["id"], correct, needs_review)
        ...
        def show_end_screen(self):
            ...
            if session.total == 0:
                ttk.Label(self.container,
                          text="No questions were answered."
        ...
        def show_procedure(self, proc):
            ...
            step_label = ttk.Label(body, text="", wraplength=900,
                                   justify="left", font=self.FONT)
            checklist_frame = ttk.Frame(body)
            check_vars = []

            def render_step():
                state["all_shown"] = False
                ...
                if i < len(steps) - 1:
                    ...
                    next_btn.state(["disabled"])
                toggle_btn.configure(text="Show full checklist")

            def render_all():
                state["all_shown"] = True
                step_label.pack_forget()
                for child in checklist_frame.winfo_children():
                    child.destroy()
                check_vars.clear()
                checklist_frame.pack(anchor="w", fill="x", pady=4)
                for i, step in enumerate(steps, 1):
                    var = tk.BooleanVar(value=False)
                    check_vars.append(var)
                    ttk.Checkbutton(checklist_frame,
                                    text="%d. %s" % (i, step),
                                    variable=var).pack(anchor="w", pady=1)
                prev_btn.state(["disabled"])
                next_btn.configure(text="All steps shown")
                next_btn.state(["disabled"])
                toggle_btn.configure(text="Back to step-by-step")
            ...
            prev_btn = ttk.Button(controls, text="< Previous step",
                                  command=back)
            prev_btn.pack(side="left")
            next_btn = ttk.Button(controls, text="Reveal next step",
                                  command=advance)
            next_btn.pack(side="left", padx=6)
            toggle_btn = ttk.Button(controls, text="Show full checklist",
                                    command=toggle)
            ...
            ttk.Button(rating, text="Back to procedures",
                       command=self.show_procedure_list).pack(side="left",
                                                              padx=6)
